...
def convert_knot_real_to_complex(dataset: CSVDataset):
    ...
    num_train, num_test = (
        dataset.data["train_input"].shape[0],
        dataset.data["test_input"].shape[0],
    )
    num_complex_input_vars = (
        len(dataset.input_varnames) - 2
    )  # -2 because there are already 2 complex numbers in the dataset (split in Re and Im)
    dataset_complex = dict()
    dataset_complex["train_input"] = torch.zeros(
        (num_train, num_complex_input_vars), dtype=torch.complex64
    )
    dataset_complex["test_input"] = torch.zeros(
        (num_test, num_complex_input_vars), dtype=torch.complex64
    )

    # copy input features into complex dataset setting imaginary part to zero (if it doesnt exist)
    for idx_orig, idx_complex in [
        (0, 0),
        (1, 1),
        (4, 3),
        (5, 4),
        ...
        dataset_complex["train_input"][:, idx_complex] = torch.complex(
            dataset.data["train_input"][:, idx_orig],
            torch.zeros_like(dataset.data["train_input"][:, idx_orig]),
        )
        dataset_complex["test_input"][:, idx_complex] = torch.complex(
            dataset.data["test_input"][:, idx_orig],
            torch.zeros_like(dataset.data["test_input"][:, idx_orig]),
        )

    # make complex number for "short geodesic"
    dataset_complex["train_input"][:, 2] = torch.complex(
        dataset.data["train_input"][:, 2], dataset.data["train_input"][:, 3]
    )
    dataset_complex["test_input"][:, 2] = torch.complex(
        dataset.data["test_input"][:, 2], dataset.data["test_input"][:, 3]
    )
    # make complex number for "meridinal translation" (in CSV imag and real are swapped...)
    dataset_complex["train_input"][:, 7] = torch.complex(
        dataset.data["train_input"][:, 9], dataset.data["train_input"][:, 8]
    )
    dataset_complex["test_input"][:, 7] = torch.complex(
        dataset.data["test_input"][:, 9], dataset.data["test_input"][:, 8]
    )
    ...
    del input_vars[9]
    del input_vars[3]
    dataset_complex = CSVDataset(
        dataset_complex,
        input_vars=input_vars,
        output_vars=["c" + str(i) for i in range(14)],
        categorical_vars=[],
    )
...
def convert_knot_complex_to_clifford(dataset: CSVDataset):
    dataset = copy.deepcopy(dataset)  # keep original dataset untouched and safe
    clifford_dict = dict()
    clifford_dict["train_input"] = torch.zeros(
        size=dataset.data["train_input"].shape + (2,)
    )
    clifford_dict["train_input"][..., 0] = dataset.data["train_input"].real
    clifford_dict["train_input"][..., 1] = dataset.data["train_input"].imag

    clifford_dict["test_input"] = torch.zeros(
        size=dataset.data["test_input"].shape + (2,)
    )
...
def run_experiments_knot(run_model, extra_args):
    ...
    for arch in [
        (in_features_real, 1, num_classes),
        (in_features_real, 2, num_classes),
    ]:
        ################################# FastKAN #################################
        if run_models[1]:
            fastkan = FastKAN(
                layers_hidden=list(arch),
                num_grids=64,
                use_batchnorm=True,
                grid_mins=-2,
                grid_maxs=2,
            ...
            run_crossval(
                ...
                knot_dataset_real,
                dataset_name="knot_r",
                loss_fn_backprop=crossentropy_loss,
                loss_fns=loss_fns,
                device=_DEVICE,
                batch_size=10000,
                logging_interval=50,
                add_softmax_lastlayer=True,
                epochs=5000,
                convert_model_output_to_real=False,
            )
    for arch in [
        (in_features_complex, 1, num_classes),
        (in_features_complex, 2, num_classes),
    ]:
        ################################# CVKAN #################################
        if run_models[2]:
            cvkan = CVKANWrapper(
...
def train_knot_feature_subset():
    ...
    for indices in [
        ...
        run_crossval(
            cvkan,
            knot_dataset_complex_reduced,
            dataset_name="knot_c_" + str(indices),
            loss_fn_backprop=crossentropy_loss,
            loss_fns=loss_fns,
            device=_DEVICE,
            batch_size=10000,
