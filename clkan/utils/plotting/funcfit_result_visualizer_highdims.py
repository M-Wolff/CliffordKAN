...
def plot_experiment_results(df, CONFIG):
    ...
    * Marker FILL    -> architecture size
    ...
    dataset_marker = {ds: m for ds, m in zip(datasets, ["o", "^", "_"])}
    ...
    random_grids = sorted(df[df[GRID] == "random_grid"][XCOL].dropna().unique())
    ...
    for i in range(len(x_bins) - 1):
        ax.axvline(
            i + 0.5, color="black", linewidth=2.5, alpha=CONFIG["sep_alpha"], zorder=0
        )
    ...
    signature_legend = [
        Line2D(
            ...
            marker="o",
            color="none",
            markerfacecolor=signature_color[s],
            markeredgecolor=signature_color[s],
            label=s,
            markersize=10,
        )
        for s in signatures
    ...
    if not zoomed:
        leg1 = fig.legend(
            ...
            labelspacing=0.2,
            ncol=3,
            bbox_to_anchor=(0.625, 1.15),
        )
        ax.add_artist(leg1)
        leg2 = fig.legend(
            handles=signature_legend,
            title="Signature",
            loc="upper right",
            labelspacing=0.2,
            ncol=4,
        ...
        ax.set_yscale("log")
        plt.savefig("funcfit_highdims.svg", bbox_inches="tight")
    else:
        ax.set_ylim(-1, 4)
        plt.savefig("funcfit_highdims_zoomed.svg", bbox_inches="tight")
    # plt.show()
