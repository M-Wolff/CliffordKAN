...
def create_clifford_dataset(name, metric, train_num): 
    ...
    if name == "square":
        ...
        dataset_dict = {"train_input": x1, "train_label": x1_squared, "test_input": torch.zeros((0,1)), "test_label": torch.zeros((0,1))}
        ...
        dataset_dict = {"train_input": x1x2, "train_label": square_of_squares, "test_input": torch.zeros((0,1)), "test_label": torch.zeros((0,1))}
    elif name == "mult":
        x1 = torch.rand((train_num,1, num_dim)) * 4 - 2
        x2 = torch.rand((train_num,1, num_dim)) * 4 - 2
        x1x2 = torch.stack((x1,x2), dim=-1)  # shape B x 1 x num_dim x num_vars
        ...
        dataset_dict = {"train_input": x1x2, "train_label": x1_mult_x2, "test_input": torch.zeros((0,1)), "test_label": torch.zeros((0,1))}
    else:
...
def synthetic_clifford(name, metric, extra_args, train_num=80000):
    ...
    dataset_dict = create_clifford_dataset(name=name, metric=metric, train_num=train_num)
    dataset_cliff = CSVDataset(dataset_dict, input_vars=input_vars[name], output_vars=output_vars[name], categorical_vars=[], train_test="100:0")
    ...
    for arch in archs[name]:
        ...
        run_crossval(cliffkan, dataset_cliff, dataset_name=f"clifford_{name}_{metric}_{train_num}", loss_fn_backprop=loss_fn_backprop,loss_fns=loss_fns, batch_size=5000, add_softmax_lastlayer=False, epochs=1000, convert_model_output_to_real=False)
