...
from .functions.ComplexSilu import complex_silu_complexweight, complex_silu_realweights, complex_silu_weight

from torch_ga.algebra import CliffordAlgebra
...
class ClVKANLayer(torch.nn.Module):
    def __init__(self, metric: list[float], input_dim: int, output_dim: int, num_grids: int = 8, grid_min = -2, grid_max = 2, rho=1, use_norm=Norms.BatchNorm, csilu_type="complex_weight"):        
        """
    ...
    def forward(self, x):
        ...
        x_expanded = x.unsqueeze(-1).unsqueeze(-1).expand(x.shape + (self.num_grids,))
        ...
        result_complex = torch.einsum("biuv,iouv->bo", result, self.weights)
        ...
        # kind of CSiLU used
        silu_value = torch.einsum("io,bi->bio",self.silu_weight, complex_silu_weight(x))
        # if self.csilu_type == "complex_weight":
        #     silu_value = torch.einsum("io,bi->bio",self.silu_weight, complex_silu_weight(x))
        # elif self.csilu_type == "real_weights":
...
class ClVKAN(torch.nn.Module):
    def __init__(self,
        ...
        self.layers = torch.nn.ModuleList([ClVKANLayer(input_dim=layers_hidden[i], output_dim=layers_hidden[i+1],
