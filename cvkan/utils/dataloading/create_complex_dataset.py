...
def save_dataset(dataset_dict, filepath):
    original_device = dataset_dict["train_input"].device
    move_dataset_to_device(dataset_dict, "cpu")
    torch.save(dataset_dict, filepath)
    move_dataset_to_device(dataset_dict, original_device)
...
def move_dataset_to_device(dataset_dict, device):
    for k in dataset_dict:
        dataset_dict[k] = dataset_dict[k].to(device)


def create_complex_dataset(f, n_var, ranges, train_num=1000, test_num=1000, device="cpu", filepath_save=None):
    """
    ...
    :param filepath_save: Where to store dataset for consistent experiment runs
    ...
    """
    if (not filepath_save is None) and filepath_save.exists():
        print(f"Using pre-computed dataset (complex-valued) {filepath_save} for data generation")
        dataset = load_dataset(filepath=filepath_save)
        move_dataset_to_device(dataset_dict=dataset, device=device)
    else:
        print(f"Pre-computed dataset (complex-valued) {filepath_save} for data generation doesn't exist. Creating it...")
